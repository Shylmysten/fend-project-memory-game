...
//@ ACTIONS. It updates the dataBlock and the UI to reflect those actions. Logic
...
//@@ Function is called when the game is won to display a modal with the
//@ User's current score and time. The modal also displays a 'no thanks' button
//@ to close the modal and a 'Play again?' button that reinitializes the game.
function openModalResetGame(event) {
  // place the overlay element into the variable 'overlay'
  const overlay = document.getElementById('overlay');
  // places the winnerModal element into the variable 'modal'
  const modal = document.getElementById('winnerModal');
  // if the item clicked is not a button
  if (!event.target.matches('button')) {
      // then we're not interested
      return;
  }
  // if the item clicked was the close( No Thanks ) button
  if (event.target.matches('.btn-close')) {
    // remove the showOverlay and showModal classes to remove the modal from
    // the users display
    overlay.classList.toggle('showOverlay');
    modal.classList.toggle('showModal');
  }
  // if the item clicked was the reset ( Play Again? ) button
  if (event.target.matches('.btn-reset')) {
      // remove the showOverlay and showModal classes to remove the modal from
      // the users display and restart the game
      overlay.classList.toggle('showOverlay');
      modal.classList.toggle('showModal');
    init();
  }
